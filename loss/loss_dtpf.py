...
class GradientMaxLoss(nn.Module):
    def __init__(self):
        ...
        # 初始化时不直接创建Parameter，而是创建register_buffer
        ...
        # 使用register_buffer替代Parameter
        self.register_buffer('sobel_x', sobel_x)
        self.register_buffer('sobel_y', sobel_y)
        self.padding = (1, 1, 1, 1)

    def forward(self, image_A, image_B, image_fuse):
        # 不需要手动移动到设备，register_buffer会自动跟随模块的设备
...
class DTPFLoss(nn.Module):
    def __init__(self, feature_weight = 1.0, base_weight = 1.0, result_weight = 1.0, cc_weight = 5.0):
        super(DTPFLoss, self).__init__()
        # 初始化融合损失组件
        ...
        self.distill_weights = {
            'feature': feature_weight,  # 特征蒸馏损失权重
            'base': base_weight,        # 基础融合损失权重
            'result': result_weight     # 结果融合损失权重
        }
        
        # 添加cc_loss权重
        self.cc_weight = cc_weight

    def compute_cc_loss(self, feats_A, feats_B):
        """计算特征解耦损失
        Args:
            feats_A: 从高层到低层的特征列表 [level1, level2, level3, level4]
            feats_B: 从高层到低层的特征列表 [level1, level2, level3, level4]
        """
        total_decomp_loss = 0
        feats_A = feats_A[::-1]
        feats_B = feats_B[::-1]
        ...
        # 计算基础特征(高层)的相关性
        cc_loss_B = cc(feats_A[0], feats_B[0])  # level4特征
        
        # 计算细节特征(低层)的相关性并构建解耦损失
        for feat_A, feat_B in zip(feats_A[1:], feats_B[1:]):  # level3-1特征
            cc_loss_D = cc(feat_A, feat_B)
            decomp_loss = cc_loss_D ** 2 / (1.01 + cc_loss_B)  # 使用基础特征相关性作为分母
            total_decomp_loss += decomp_loss
        
        return total_decomp_loss / (len(feats_A) - 1)  # 取平均

    def forward(self, image_A, image_B, image_fused, image_fused_distilled=None, stage='parent', text=None, 
                teacher_feats=None, student_feats=None, encoder_A_feats=None, encoder_B_feats=None):
        ...
        if stage == 'parent':
            # 父模型训练阶段
            tasks = text_to_task_list(text)  # 从text生成任务列表
            ...
            for batch_idx in range(batch_size):
                ...
                for task in sample_tasks:
                    ...
                    consist_ratio = config.get('consist_ratio', 0)
                    text_ratio = config.get('text_ratio', 0)
                    
                    if max_ratio > 0 or consist_ratio > 0:
                        loss, ssim, max_l, color_l, grad_l = self.compute_fusion_loss(**loss_args)
                        task_loss = (
                            max_ratio * max_l + 
                            consist_ratio * grad_l + 
                            text_ratio * color_l
                        )
                        sample_loss = sample_loss + task_loss
            ...
            # 添加特征解耦损失
            if encoder_A_feats is not None and encoder_B_feats is not None:
                cc_loss = self.compute_cc_loss(encoder_A_feats, encoder_B_feats)
                total_loss = total_loss + self.cc_weight * cc_loss
            ...
            # 添加parent阶段的特征解耦损失
            if encoder_A_feats is not None and encoder_B_feats is not None:
                cc_loss_parent = self.compute_cc_loss(encoder_A_feats, encoder_B_feats)
                fusion_loss_parent = fusion_loss_parent + self.cc_weight * cc_loss_parent
                loss_dict['cc_loss_parent'] = cc_loss_parent
            ...
            if image_fused_distilled is not None:
                ...
                # 添加student阶段的特征解耦损失
                if encoder_A_feats is not None and encoder_B_feats is not None:
                    cc_loss_distill = self.compute_cc_loss(encoder_A_feats, encoder_B_feats)
                    fusion_loss_distill = fusion_loss_distill + self.cc_weight * cc_loss_distill
                    loss_dict['cc_loss_distill'] = cc_loss_distill
    ...
    def compute_fusion_loss(self, image_visible, image_infrared, image_fused, max_ratio=4, consist_ratio=1, 
                            ssim_ir_ratio=1, ssim_ratio=1, ir_compose=1, color_ratio=12, text_ratio=10, 
